/**
 * Created by Michael on 11/11/2016. Used Implementation provided by Stefan Gustavson Based on Ken Perlin's Improved Noise, Simplex Noise
 */
public class Simplex {
	...
	private int[] p = { 151, 160, 137, 91, 90, 15, 131, 13, 201, 95, 96, 53, 194, 233, 7, 225, 140, 36, 103, 30, 69, 142, 8, 99, 37, 240, 21, 10, 23, 190, 6, 148, 247, 120, 234, 75, 0, 26, 197, 62, 94, 252, 219, 203, 117, 35, 11, 32, 57, 177, 33, 88, 237, 149, 56, 87, 174, 20, 125, 136, 171, 168, 68, 175, 74, 165, 71, 134, 139, 48, 27, 166, 77, 146, 158, 231, 83, 111, 229, 122, 60, 211, 133, 230, 220, 105, 92, 41, 55, 46, 245, 40, 244, 102, 143, 54, 65, 25, 63, 161, 1, 216, 80, 73, 209, 76, 132, 187, 208, 89, 18, 169, 200, 196, 135, 130, 116, 188, 159, 86, 164, 100, 109, 198, 173, 186, 3, 64, 52, 217, 226, 250, 124, 123, 5, 202, 38, 147, 118, 126, 255, 82, 85, 212, 207, 206, 59, 227, 47, 16, 58, 17, 182, 189, 28, 42, 223, 183, 170, 213, 119, 248, 152, 2, 44, 154, 163, 70, 221, 153, 101,
	...
	public double domainWarp(int iterations, int x, int y, double persistance, double scale, int low, int high, int xOffs, int yOffs, int warpIntensity) {
		...
		System.out.println(warpIntensity);

		int v1 = 4 * (int) sumOctave(iterations, x + yOffs, y + xOffs, persistance, scale, low, high, xOffs, yOffs);
		int v2 = 4 * (int) sumOctave(iterations, y + xOffs, x + yOffs, persistance, scale, low, high, xOffs, yOffs);

		if (warpIntensity == 1) {
			return sumOctave(iterations, x + v1, y + v2, persistance, scale, low, high, xOffs, yOffs);
		} else if (warpIntensity == 2) {

			int q1 = (int) sumOctave(iterations, x + v1, y + v1, persistance, scale, low, high, xOffs, yOffs);
			int q2 = (int) sumOctave(iterations, x + v2, y + v2, persistance, scale, low, high, xOffs, yOffs);

			return (int) sumOctave(iterations, x + q1, y + q2, persistance, scale, low, high, xOffs, yOffs);
		} else if (warpIntensity == 3) {

			int q1 = (int) sumOctave(iterations, x + v1, y + v1, persistance, scale, low, high, xOffs, yOffs);
			int q2 = (int) sumOctave(iterations, x + v2, y + v2, persistance, scale, low, high, xOffs, yOffs);

			int u1 = (int) sumOctave(iterations, x + q1, y + q1, persistance, scale, low, high, xOffs, yOffs);
			int u2 = (int) sumOctave(iterations, x + q2, y + q2, persistance, scale, low, high, xOffs, yOffs);

			return (int) sumOctave(iterations, x + u1, y + u2, persistance, scale, low, high, xOffs, yOffs);
		} else if (warpIntensity == 4) {
			
			int q1 = 8 * (int) sumOctave(iterations, x + v1, y + v2, persistance, scale, low, high, xOffs, yOffs);
			int q2 = 16 * (int) sumOctave(iterations, x + v1, y + v2, persistance, scale, low, high, xOffs, yOffs);

			int u1 = 32 * (int) sumOctave(iterations, x + v1, y + v2, persistance, scale, low, high, xOffs, yOffs);
			int u2 = 64 * (int) sumOctave(iterations, x + q1, y + q2, persistance, scale, low, high, xOffs, yOffs);
	...
	public double domainWarp(int iterations, int x, int y, int z, double persistance, double scale, int low, int high, int xOffs, int yOffs, int warpIntensity) {
		...
		if (warpIntensity == 1) {
			...
			int q1 = (int) sumOctave(iterations, x + v1, y + v1, z, persistance, scale, low, high, xOffs, yOffs);
			int q2 = (int) sumOctave(iterations, x + v2, y + v2, z, persistance, scale, low, high, xOffs, yOffs);

			int u1 = (int) sumOctave(iterations, x + q1, y + q1, z, persistance, scale, low, high, xOffs, yOffs);
			int u2 = (int) sumOctave(iterations, x + q2, y + q2, z, persistance, scale, low, high, xOffs, yOffs);

			return (int) sumOctave(iterations, x + u1, y + u2, 0, scale, low, high, xOffs, yOffs);
		} else if (warpIntensity == 4) {

			int q1 = (int) sumOctave(iterations, x + v1, y + v2, z, persistance, scale, low, high, xOffs, yOffs);
			int q2 = (int) sumOctave(iterations, x + v1, y + v2, z, persistance, scale, low, high, xOffs, yOffs);

			int u1 = (int) sumOctave(iterations, x + q1, y + q2, z, persistance, scale, low, high, xOffs, yOffs);
			int u2 = (int) sumOctave(iterations, x + q1, y + q2, z, persistance, scale, low, high, xOffs, yOffs);

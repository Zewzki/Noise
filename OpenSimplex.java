/**
 * Created by Michael on 11/13/2016. Written by Kurt Spencer Placed in Public
 * Domain October 5, 2014
 */
public class OpenSimplex {
	...
	private int noiseType;// 0 = standard
	...
	public double sumOctave(int iterations, double x, double y, double persistance, double scale, int low, int high,
			int xOffs, int yOffs) {
		...
		for (int i = 0; i < iterations; i++) {
			double adding = eval((x + xOffs) * freq, (y + yOffs) * freq) * amp;
			noise += adding;
			maxAmp += amp;
			amp *= persistance;
			freq *= 2;
	...
	public double sumOctave(int iterations, double x, double y, double z, double persistance, double scale, int low,
			int high, int xOffs, int yOffs) {
		...
		for (int i = 0; i < iterations; i++) {
			double adding = eval((x + xOffs) * freq, (y + yOffs) * freq, z * freq) * amp;
			noise += adding;
			maxAmp += amp;
			amp *= persistance;
			freq *= 2;
